...
Inherited contracts: [main.spec.md](../../main.spec.md) §Cross-Module Contracts (enrichment-result shape at line 22; `Error("Could not respond")` failure signal at line 24).
...
- **F5 — Failure signal.** On repair exhaustion (or initial failure when `repair_fn is None`), the module raises `Error("Could not respond")` — a module-local `Exception` subclass whose name matches [main.spec.md §Cross-Module Contracts](../../main.spec.md) line 24 literally. The precise underlying diagnostic is preserved via `__cause__`.
...
- **NF5 — `Error` name discipline.** The exception class is named `Error` per [main.spec.md:24](../../main.spec.md) literally. Callers MUST import it explicitly (`from person_finder.validation import Error`) rather than via star-import to avoid namespace pollution. Documented in the module docstring.
...
- **AC5** — Each failure mode (JSON parse error; top-level not dict; missing `"data"` key; `data` not list; `data[i]` not dict; missing `"person"` / `"info"` key; non-str `"person"` / `"info"`) yields a precise diagnostic delivered to `repair_fn` via its `error_msg` argument. Currently verified by the parametrized `test_repair_fn_receives_precise_error_msg` (9 cases) plus `test_repair_fn_returning_non_str_is_a_validation_failure` for the defensive non-str case.
...
| OQ2 | Whether `Error("Could not respond")` should optionally carry the last broken raw for user-facing diagnostics, or stay terse per the literal contract. | NF5 / [main.spec.md:24](../../main.spec.md). |
